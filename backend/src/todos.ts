import { Router, Request, Response } from "express";
import { PrismaClient } from "@prisma/client";
import { z } from 'zod';
const prisma = new PrismaClient();
const router = Router();


const Todo = z.object({
    tid: z.number().int(), // Ensure tid is an integer
    id: z.string(), // Assuming id is a string or a Date string
    task: z.string().min(1), // Ensure task is a non-empty string
    complete: z.boolean(), // Boolean indicating task completion
  });
  

  // for testing purpose only
  router.get("/", (req: Request, res: Response) => {
    res.send("hey you are in add now");
  });
  
  
  
  // Route to add a new Todo
  router.post("/todos", async (req: Request, res: Response) => {
    // const { success, error } = Todo.safeParse(req.body);
  
    // if (!success) {
    //   return res.status(411).json({
    //     message: "Incorrect inputs",
    //     error: error?.issues, // Optional: Include the validation issues for debugging
    //   });
    // }
  
    const { id, task, complete } = req.body;
  
    try {
      const newTodo = await insertTodo(id, task, complete); // Await the result of insertTodo
      console.log("Result:", newTodo);
      res.status(201).json(newTodo);
    } catch (err) {
      console.error("Error in /todos POST:", err);
      res.status(500).json({ error: "Internal Server Error" });
    }
  });
  
  // Route to get all Todos
  router.get("/todos", async (req: Request, res: Response) => {
    try {
      const todos = await prisma.todo.findMany();
      res.json(todos);
    } catch (err) {
      console.error("Unable to fetch Todos:", err);
      res.status(500).json({ error: "Internal Server Error" });
    }
  });





// Function to insert a new Todo into the database
async function insertTodo(id: Date, task: string, complete: boolean) {
    try {
      const newTodo = await prisma.todo.create({
        data: {
          id,
          task,
          complete,
        },
        select: {
          id: true,
          tid: true,
          task: true,
          complete: true,
        },
      });
      console.log("Inserted Todo:", newTodo);
      return newTodo; // Return the inserted Todo
    } catch (err) {
      console.error("Unable to insert into the database:", err);
      throw err; // Re-throw the error for proper handling
    }
  }


  export default router;